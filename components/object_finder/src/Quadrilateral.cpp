...
array<float, 4> Quadrilateral::GetSides()
{
    ...
    return {_GetDistance(arr[0], arr[1]),
            _GetDistance(arr[1], arr[2]),
            _GetDistance(arr[2], arr[3]),
            _GetDistance(arr[3], arr[0])};
}
...
float Quadrilateral::GetArea()
{
    ...
    float result = sqrt((semip - s[0]) * (semip - s[1]) * (semip - s[2]) * (semip - s[3]));
    return isnan(result) ? 0 : result;
}

Point2f &Quadrilateral::operator[](int i)
{
    return arr[i];
}

const Point2f &Quadrilateral::operator[](int i) const
{
    return arr[i];
}
