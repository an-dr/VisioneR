...
App::App(FaceInterface *face, InputInterface *input, Gui *gui,
         InterfaceSceneReader *scene_reader)
    : m_face(face), m_input(input), m_scene_input(scene_reader), m_gui(gui),
      m_objectFinder(new ObjectFinder), m_vis(new Visualizer) {}
...
cv::Mat App::GetScene() { return m_objectFinder->GetScene(); }

QImage App::ToQImage(cv::Mat &cv_mat) {
    // cv::Mat temp;
    // cv_mat.copyTo(temp);
    // cv::cvtColor(temp, temp, cv::COLOR_BGR2RGB);
    return QImage((unsigned char *)cv_mat.data, cv_mat.cols, cv_mat.rows,
